...
When the **uploadMode** is *"useForm"*, define the parameters within hidden inputs. They are sent to the server in an HTML form along with the files. Some DevExtreme UI components have underlying hidden inputs too. Use the widget's **name** option to specify the input's `name` attribute. 
