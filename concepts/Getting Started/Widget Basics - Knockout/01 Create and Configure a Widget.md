...
All DevExtreme UI components can be created on a page in the same manner - using a widget-specific Knockout binding. To create, for example, the dxChart widget, add a `<div>` element to the `<body>` tag of your page and use the `dxChart` binding as the following code shows.

    <!--HTML--><div data-bind="dxChart: { }"></div>

To configure a widget, add properties to the object passed to the UI component binding. Note that these properties mirror the options of the widget.
